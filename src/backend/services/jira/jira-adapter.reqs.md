...
- `src/backend/types` -> `JiraTicketData`, `JiraTransition`, `JiraStatus`, `ProjectConfig`, `JiraApiError`, `TicketNotFoundError`, `PermissionDeniedError`, `TransitionBlockedError`, `TimeoutError`
...
- Mocked `@forge/api` `requestJira` for all 7 functions
