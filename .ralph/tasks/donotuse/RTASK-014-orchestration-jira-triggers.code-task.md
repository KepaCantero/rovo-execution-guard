...
priority: 3
type: orchestration
dependencies: [RTASK-008, RTASK-009, RTASK-010, RTASK-013, RTASK-021]
...
Logic in dedicated handlers under `src/backend/handlers/`

### Trigger Flow
1. Jira fires transition event
2. Handler gets ticket data via Jira adapter
3. Gets context via Rovo adapter
4. Evaluates corresponding Quality Gate
5. If fails: blocks transition + adds comment with reasons
6. If passes: allows transition
7. Logs to audit
...
|------------------|-------------------|-----------------|
| `src/backend/handlers/workflow-trigger.ts` | `src/backend/handlers/workflow-trigger.reqs.md` | `tests/unit/handlers/workflow-trigger.spec.ts` |
...
- [ ] **GATE-READY**: All dependencies (RTASK-008, RTASK-009, RTASK-010, RTASK-013, RTASK-021) are completed
