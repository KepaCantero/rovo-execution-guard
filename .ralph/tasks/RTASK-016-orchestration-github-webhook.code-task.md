...
priority: 3
type: orchestration
dependencies: [RTASK-008, RTASK-011, RTASK-009, RTASK-010, RTASK-013, RTASK-017]
...
- Register as a **webtrigger** in `manifest.yml` (key: `github-webhook`, function: `github-webhook-fn`)
- Handler MUST be at `src/backend/resolvers/github-webhook.handler` (path declared in manifest.yml function `github-webhook-fn`)
- Listen for GitHub PR events via incoming HTTP requests
- Endpoint must handle POST requests with JSON payloads

> **IMPORTANT**: The handler file path MUST match `manifest.yml` exactly. The manifest declares `handler: src/backend/resolvers/github-webhook.handler`. Do NOT create the handler at `src/backend/handlers/`.
...
receive webhook
  ...
  -> for each Jira key:
       ...
       -> detect inconsistencies + calculate score
       -> evaluate Quality Gate via quality-gate-rules
       -> call enforcement functions from RTASK-017:
          - On fail: blockPR(), addComment() with formatted Markdown
          - On pass: approvePR(), addComment() with success details
...
| ------------------------------------------------ | ----------------------------------------------------- | ----------------------------------------------------- |
| `src/backend/resolvers/github-webhook.ts`        | `src/backend/resolvers/github-webhook.reqs.md`        | `tests/unit/resolvers/github-webhook.spec.ts`         |
...
- [ ] **GATE-READY**: All dependencies (RTASK-008, RTASK-011, RTASK-009, RTASK-010, RTASK-013, RTASK-017) are completed
